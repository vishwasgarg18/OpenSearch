...
use crate::stats::BlockCacheStats;
    ...
    fn on_leave(&self, reason: Event, key: &String, value: &Vec<u8>) {
        ...
        match reason {
            ...
            Event::Clear => {
                ...
                // Reset used_bytes — everything was removed.
...
pub struct FoyerCache {
    ...
    pub(crate) stats: Arc<BlockCacheStats>,
}
...
impl PageCache for FoyerCache {
    async fn get(&self, key: &CacheKey) -> Option<Bytes> {
        match self.inner.get(&key.as_str().to_string()).await {
            ...
            _ => {
                ...
                // The requested range size is encoded in the key (format "path\x1Fstart-end"),
                // so miss_bytes is known before any remote fetch — no callback needed.
    ...
    async fn clear(&self) {
        ...
        // used_bytes is reset by KeyIndexListener::on_leave(Event::Clear)
    }
}
