...
//!
//! [`BlockCacheStats`] is the Rust equivalent of Java's
//! `DefaultStatsCounter`/`IRefCountedCacheStats` pattern.
//!
...
//!   [`BlockCacheStats::snapshot()`] called from the `foyer_snapshot_stats` FFM
//!   function in [`crate::foyer::ffm`].
//!
//! The snapshot produces an `[i64; 5]` array whose layout is fixed and
//! documented on [`BlockCacheStats::snapshot`]. Java reads this array via
