 ...
 * is seeded from remote metadata at shard open.
 *
 * <p><b>TODO (writable warm):</b> add getFileLocation, acquireRead, releaseRead
 * when LOCAL parquet files exist and eviction is enabled.
 */
public class ParquetDataFormatStoreHandler implements DataFormatStoreHandler {
    ...
    public ParquetDataFormatStoreHandler(
        ...
        if (isWarm) {
            ...
            // Resolve preferred cache by name; 0L if unavailable (hot nodes or no matching cache).
            long cachePtr = (cacheRegistry != null)
                ? cacheRegistry.get(BuiltInBlockCaches.FOYER)
                    .map(BlockCache::nativeBlockCachePtr)
                    .orElse(0L)
                : 0L;
            ...
            logger.debug("[{}] ParquetDataFormatStoreHandler created: cache={}",
                shardId, cachePtr != 0L ? BuiltInBlockCaches.FOYER : "none");
        } else {
