...
public class UnifiedCacheService implements Closeable {
     ...
     *
     * <ul>
     *   <li>If no block cache is configured: returns {@link FileCache#cacheStats()}
     *       unchanged — fully backwards compatible.</li>
     *   <li>If a block cache IS configured: calls {@link BlockCacheHandle#cacheStats()}
     *       (which performs an FFM call to read Rust atomic counters) and combines
     *       the result via {@link AggregateFileCacheStats#add}.
     *       Sections present in one but not the other (e.g. {@code full_file_stats},
     *       {@code pinned_file_stats}) are carried through unchanged.</li>
     * </ul>
     *
